...
def order_detail(request, order_id):
    if hasattr(request.user, "employee"):
        ...
        if request.method == "POST" and "mark_as_accepted" in request.POST:
            order.status = "accepted"
            order.completion_date = timezone.now()
            order.save()
            messages.success(request, "Заказ принят к исполнению")
            return redirect("catalog:employee_dashboard")
    else:
        ...
        elif 'change_quantity' in request.POST:
            new_quantity = request.POST.get('quantity')
            new_quantity = int(new_quantity)
            if new_quantity < order.product.min_order_quantity:
                    return render(request, 'catalog/order_detail.html', {
                        'order': order,
                        'error': f"Количество должно быть не менее {order.product.min_order_quantity}."
                    })
...
def create_order(request):
    ...
    products = Product.objects.filter(is_active=True)
    return render(request, "catalog/create_order.html", {"products": products, 'selected_product_id': selected_product_id,})
...
def employee_dashboard(request):
    ...
    top_clients = assigned_clients.annotate(
        total_orders=Sum("order__total_amount",filter=Q(order__status="accepted"))
...
def payment_view(request, order_id):
    ...
    return render(request, "catalog/payment.html", {"order": order})